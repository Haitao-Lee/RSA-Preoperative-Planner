...
void vtkImageInteractionCallback::Execute(vtkObject* caller, unsigned long eventId, void* callData)
{
	if (img_ornot)
	{
		...
		if (eventId == vtkCommand::LeftButtonPressEvent)
		{
			...
			if (normal[0])
			{
				this->common_style->callback1->synWithOtherCallback(this->actor_center[1] - picked[1]);
			...
			else if (normal[1])
			{
				...
				this->common_style->callback1->Interactor->Render();
				this->common_style->callback2->synWithOtherCallback(this->actor_center[1] - picked[0]);
